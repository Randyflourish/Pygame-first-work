...
pygame.display.set_caption("戴達洛斯——竹君的異界之旅  made by 羅鵬博、江柏廷、王藝翔")  # 程式名稱
...
def round_start():
    ...
    for i in range(len(variable.usecha)):
        ...
        if perform.nHP > 0:
            ...
            if perform.recover > 0:
                perform.recover -= 1
                perform.nHP += max(int((perform.HP[perform.level]+perform.HPlimit)
                                       * 0.05), 1)
                perform.nHP = min(
                    perform.nHP, perform.HP[perform.level]+perform.HPlimit)
    ...
    for i in range(len(variable.enemy)):
        ...
        if ene.nHP > 0:
            ...
            if ene.recover > 0:
                ene.recover -= 1
                ene.nHP += max(int((ene.HP[ene.level]+ene.HPlimit)
                                   * 0.05), 1)
                ene.nHP = min(
...
def ene_ATK_choose():
    ...
    # 攻擊對象選取
    for i in range(len(variable.enemy)):
        object = random.randint(0, len(variable.usecha)-1)
...
bag_us_font_1 = font_27_80_B.render("萬能輝星", False, BLUE_VIOLET)
...
class fn_pn(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.pnd_detal_HP = font_27_30_B.render(
            'HP：'+str(self.perform.nHP)+' / '+str(self.perform.HP[self.perform.level]+self.perform.HPlimit), False, BLACK)
    ...
    def update(self):
        ...
        if self.rect.collidepoint(mouse_location) and mouse_one_press(0):
            if not flag.attack_choose:
                ...
                flag.attack_choose = False
                flag.sprite_need_change = True
        elif keyboard_one_press(pygame.K_LEFT):
            sprite.act_information.act -= 1
            if sprite.act_information.act < 0:
                sprite.act_information.act = len(variable.usecha)
            # battle_large_image.h_update(照片)
        elif keyboard_one_press(pygame.K_RIGHT):
            sprite.act_information.act += 1
            if sprite.act_information.act > len(variable.usecha):
                sprite.act_information.act = 0
            # battle_large_image.h_update(照片)
...
class HPbar(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.num <= 2:
            self.rect.topleft = (
                chaloc[variable.usecha[self.num]][0]-1, chaloc[variable.usecha[self.num]][1]+159)
                ...
                if self.act <= 2:  # 我方
                    if self.perform.nHP != 0 and self.perform.dizz == 0:  # 還活著、沒暈眩
                        ...
                        if variable.myATKtype[self.act] == 0:  # 普攻
                            ...
                            if self.act == 0 and self.skilling > 0:
                                if self.perform.upskill:
                                ...
                                if len(self.obj) == 1:  # 單一目標
                                    ...
                                    if self.act == 0 and self.w.code == (1, 1):
                                        ...
                                        self.perform.nHP = min(
                                            self.perform.nHP, self.perform.HP[self.perform.level]+self.perform.HPlimit)
                                        self.str3 = "天秤回正，回復 " + \
                                    ...
                                    if o.type == '奧利哈鋼魔像' and o.dizz == 0 and o.nHP != 0:
                                        antiatk = max(int(cause_dmg)*0.1, 1)
